...
function SortableHostCard({ agent }: SortableHostCardProps) {
  ...
  return (
    <div
      ...
      style={style}
      {...attributes}
      className={`group bg-bg-card rounded-xl border border-purple-500/10 overflow-hidden shadow-soft hover:shadow-glow transition-shadow ${isDragging ? 'shadow-glow' : ''}`}
    >
      {/* Main row */}
      <div
            ...
            <div className="space-y-3">
              ...
              {lastSeen && (
                <div className="text-xs text-gray-500">
                  最后上报: {lastSeen.toLocaleString('zh-CN')}
                </div>
              )}
            </div>
          ) : (
...
function GridHostCard({ agent }: SortableHostCardProps) {
  ...
  return (
    <div
      className="bg-bg-card rounded-xl border border-purple-500/10 p-4 flex flex-col gap-3 shadow-soft hover:shadow-glow hover:border-accent/20 transition-all cursor-pointer"
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1800px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6">
        ...
        <div className="flex items-center gap-1 bg-bg-hover rounded-lg p-1">
          <button
            ...
            className={`p-1.5 rounded-md transition-colors ${viewMode === 'list' ? 'bg-bg-card text-accent shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
            title="列表视图"
            aria-label="列表视图"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-bg-card text-accent shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
            title="卡片视图"
            aria-label="卡片视图"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <div className="bg-bg-card rounded-xl border border-purple-500/10 p-3 shadow-soft">
