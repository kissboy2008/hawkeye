...
function AddWidgetDialog({ open, onClose, onSave, existingGroups }: { open: boolean; onClose: () => void; onSave: (w: Partial<Widget>) => void; existingGroups: string[] }) {
  ...
  const typeOptions = [
    ...
    { value: 'pbs', label: 'Proxmox Backup Server', placeholder: 'https://192.168.1.99:8007' },
    { value: 'portainer', label: 'Portainer', placeholder: 'https://192.168.1.100:9443' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-bg-card rounded-xl p-6 w-full max-w-md border border-purple-500/10 shadow-glow">
        ...
        <div className="space-y-3">
          <div>
            ...
            <select
              className="w-full bg-bg-hover border border-purple-500/10 rounded-lg px-3 py-2 text-sm"
              value={form.type}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <select
                className="flex-1 bg-bg-hover border border-purple-500/10 rounded-lg px-3 py-2 text-sm"
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <button
            onClick={() => {
            const config = form.url ? JSON.stringify({ link_url: form.url }) : ''
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-bg-card rounded-xl p-6 w-full max-w-md border border-purple-500/10 shadow-glow">
        ...
        <div className="space-y-3">
          ...
          {['proxmox', 'pbs'].includes(widget.type) && (
          <div>
          ...
          <div>
            <label className="text-xs text-gray-400 block mb-1">跳转地址（可选）</label>
            <input
              className="w-full bg-bg-hover border border-purple-500/10 rounded-lg px-3 py-2 text-sm"
              placeholder="点击标题跳转的 URL，如 https://192.168.1.200:8006"
              value={linkUrl}
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <button
            onClick={() => {
              const configObj = { ...parsedConfig, link_url: linkUrl || undefined }
...
function WidgetContextMenu({ x, y, onEdit, onDelete, onClose }: { x: number; y: number; onEdit: () => void; onDelete: () => void; onClose: () => void }) {
  return (
    <>
      ...
      <div
        className="fixed z-50 bg-bg-card border border-purple-500/10 rounded-lg shadow-glow py-1 min-w-[120px] animate-in fade-in zoom-in-95 duration-100"
        style={{ left: x, top: y }}
      >
        <button
          onClick={onEdit}
...
function EmptyGroupDropZone({ groupName }: { groupName: string }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`border-2 border-dashed rounded-xl py-12 text-center text-xs transition-colors ${
        isOver ? 'border-blue-400 bg-blue-500/10 text-blue-400' : 'border-purple-500/10 text-gray-600'
      }`}
    >
      拖拽组件到此处
    </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 text-sm rounded-lg transition-all ${editMode ? 'bg-blue-500/20 text-blue-400 border border-blue-500/50' : 'bg-white/5 text-gray-400 border border-purple-500/10 hover:text-gray-200'}`}
          >
            {editMode ? '✓ 完成排列' : '⇄ 排列组件'}
          </button>
          {editMode && (
            <button
              ...
              className={`px-4 py-2 text-sm rounded-lg transition-all ${
                groups.length >= 4
                  ? 'bg-white/5 text-gray-600 border border-purple-500/10 cursor-not-allowed'
